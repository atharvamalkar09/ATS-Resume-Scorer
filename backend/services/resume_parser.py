import io
import magic  # checks the %pdf -> internally
from typing import Tuple,Optional

import pdfplumber       # parses pdf
from docx import Document   # parses document
import PyPDF2               # Reads the data from the pdf
...
def validate_file(file_data:bytes,filename:str) -> Tuple[bool,str,Optional[str]]:
    ...
    if file_size_bytes==0:
        return False, 'uploade file is empty...please check the file you have uploaded and try again',None   # added none myself
    ...
    if mime_type not in SUPPORTED_MIME_TYPES:
        supported=', '.join(SUPPORTED_MIME_TYPES.keys()).upper()   # .values() instaed -> to show actual pdf,doc,docx
        return False, (
...
## PDF bytes → Open PDF → Loop pages → Check /Annots → Loop annotations → Check /Link → Extract URI → Decode (if bytes) → Strip spaces → Check "http" → Add URL → Join with '\n' → Return
...
# Main Parsing and returning text from the file is done here...
